...
import { fontWeights, getWeightLabel } from "~/data/fontWeights";
...
export default function TextRow({ layer, onUpdateLayer }: TextRowProps) {
  ...
  const [lineHeightValue, setLineHeightValue] = React.useState(layer?.lineHeight ?? 1);
  const [letterSpacingValue, setLetterSpacingValue] = React.useState(layer?.letterSpacing ?? 0);

  React.useEffect(() => {
    setLineHeightValue(layer?.lineHeight ?? 1);
      ...
      <div className="flex flex-row items-center justify-between">
        <div className="flex w-full flex-row gap-1.5">
          ...
          <div className="w-[88px] flex-shrink-0">
            <Combobox
              size="xsmall"
              value={layer?.fontSize?.toString() || "16"}
              onValueChange={(value) =>
        ...
        <div className="flex w-[154px] flex-row gap-1.5">
          <Input.Root>
            <Input.Wrapper iconPosition="right">
              <Input.Input
                type="number"
                value={Math.round(lineHeightValue * (layer?.fontSize ?? 16))}
                onChange={(e) => {
                  const value = parseInt(e.target.value);
                  if (value >= 0) {
                    setLineHeightValue(value / (layer?.fontSize ?? 16));
                  }
                }}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
